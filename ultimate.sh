...
PKGEOF
    ...
    echo "1. Configure bot: ./ultimate.sh (option 2)"
    echo "2. Start bot: ./ultimate.sh (option 4)"
...
BANNER
    ...
    if [ -d "node_modules" ]; then
        echo -e " ${GREEN}✓ Installed${NC}"
        echo -e "   ${BLUE}Packages:${NC} $(ls node_modules | wc -l)"
    else
    ...
    if [ -f "src/data/users.json" ]; then
        users=$(cat src/data/users.json | grep -o "{[^{}]*}" | wc -l)
    ...
    if command -v pm2 &> /dev/null; then
        if pm2 list 2>/dev/null | grep -q "VaultHeist"; then
            ...
            pm2 show VaultHeist 2>/dev/null | grep -E "status|restarts|uptime"
        else
